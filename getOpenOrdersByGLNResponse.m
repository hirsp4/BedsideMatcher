...
@implementation getOpenOrdersByGLNResponse

@synthesize items;

+ (getOpenOrdersByGLNResponse *)createWithXml:(DDXMLElement *)__node __request:(RequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(RequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(order*)[__request createObject:child type:[order class]]];
        }
...
-(void) serialize:(DDXMLElement*)__parent __request:(RequestResultHandler*) __request
{
    for (order* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[order class] name:@"return" URI:@"" parent:__parent skipNullProperty:NO];
...
- (NSArray *)arrayByAddingObject:(order*)anObject {
...
- (BOOL)containsObject:(order*)anObject {
...
- (NSUInteger)indexOfObject:(order*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(order*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(order*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(order*)anObject inRange:(NSRange)range {
...
- (void)addObject:(order*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(order*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(order*)anObject {
...
- (void)removeObject:(order*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(order*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(order*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(order*)anObject {
...
+ (id)array {
	return [[ getOpenOrdersByGLNResponse alloc] init];
}

+ (id)arrayWithObject:(order*)anObject {
	return [[ getOpenOrdersByGLNResponse alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const order* *)objects count:(NSUInteger)cnt {
	return [[ getOpenOrdersByGLNResponse alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(order*)firstObj, ... {
	getOpenOrdersByGLNResponse* a = [ getOpenOrdersByGLNResponse array];
	id eachObject;
...
- (id)initWithObjects:(order*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ getOpenOrdersByGLNResponse alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ getOpenOrdersByGLNResponse alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ getOpenOrdersByGLNResponse alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ getOpenOrdersByGLNResponse alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const order* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(order*, order*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(order*, order*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(order*, order*, void *))compare context:(void *)context {
